    ...
    <div className="min-h-screen w-full bg-background pb-20">
      ...
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-md border-b border-border">
        <div className="flex items-center justify-between px-6 py-4">
          <h1 className="text-2xl font-cooper font-bold" style={{ color: '#A88AED' }}>
            Мои образы
          </h1>
          <div className="flex items-center gap-2">
            <Heart className="w-5 h-5 fill-primary text-primary" />
            <span className="text-sm font-cooper font-semibold" style={{ color: '#A88AED' }}>{savedOutfits.length}</span>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-6">
        {savedOutfits.length === 0 ? (
          <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
            ...
            <h2 className="text-xl font-cooper font-semibold mb-2" style={{ color: '#A88AED' }}>
              Пока нет сохраненных образов
            </h2>
            <p className="font-cooper" style={{ color: '#A88AED' }}>
            ...
            {savedOutfits.map((outfit) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-3">
                  <p className="text-sm font-cooper font-semibold" style={{ color: '#A88AED' }}>
                    {outfit.occasion}
                  </p>
                </div>
              </div>
